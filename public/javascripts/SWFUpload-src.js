...
SWFUpload.prototype.loadFlash = function() {
 ...
 var container = $div({id: 'flashContainer'});
...
function fileQueued(file, queuelength) {
 ...
 li.innerHTML = file.name + " <div class='progressBar' id='" + file.id + "progress'><!-- --></div><a id='" + file.id + "deletebtn' class='cancelbtn' href='javascript:swfu.cancelFile(\"" + file.id + "\");'><!-- IE --></a>";
...
function uploadFileCancelled(file, queuelength) {
 var li = $(file.id);
 li.update(file.name + " - cancelled");
 li.className = "SWFUploadFileItem uploadCancelled";
}

function uploadFileStart(file, position, queuelength) {
 $("queueinfo").update("Uploading file " + position + " of " + queuelength);
 $(file.id).addClassName("fileUploading");
}
...
function uploadQueueComplete(file) {
 $("queueinfo").update("The file has been uploaded. One moment please...");
 $("cancelqueuebtn").hide();
}
