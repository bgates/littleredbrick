module WhiteListHelper
            ...
            unless tags.include?(node.name)
              ...
              if node.closing != :close
                node.attributes.delete_if do |attr_name, value|
                  !attrs.include?(attr_name) || (PROTOCOL_ATTRIBUTES.include?(attr_name) && has_bad_protocols?(value))
  ...
  protected
    def has_bad_protocols?(value)
      value =~ PROTOCOL_SEPARATOR && !white_listed_protocols.include?(value.split(PROTOCOL_SEPARATOR).first)
    end
end

WhiteListHelper.bad_tags   = %w(img script)
WhiteListHelper.tags       = %w(strong em b i p code pre tt output samp kbd var sub sup dfn cite big small address hr br div span h1 h2 h3 h4 h5 h6 ul ol li dt dd abbr acronym a blockquote del ins fieldset legend)
