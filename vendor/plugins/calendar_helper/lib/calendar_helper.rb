...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    prev, prev_title = if options[:month] == 1
      [Date.civil(options[:year] - 1, 12,1), Date::MONTHNAMES[12]]
    else
      [Date.civil(options[:year], options[:month] - 1, 1), Date::MONTHNAMES[options[:month] - 1]]
    end
    nex, next_title = if options[:month] == 12
      [Date.civil(options[:year] + 1, 1,1), Date::MONTHNAMES[1]]
    else
      [Date.civil(options[:year], options[:month] + 1, 1), Date::MONTHNAMES[options[:month] + 1]]
    end
    ...
    cal << %(<caption class="#{options[:month_name_class]}"></caption><thead>)
    cal << %(<tr  class="#{options[:day_name_class]}">)
    day_names.each do |d|
      unless d[options[:abbrev]].eql? d
        cal << "<th scope='col'"
        cal << " class='weekendDay'" if ['Sunday', 'Saturday'].include?(d)
        cal << "><abbr title='#{d}'>#{d[options[:abbrev]]}</abbr></th>"
      else
        cal << "<th scope='col'"
        cal << " class='weekendDay'" if ['Sunday', 'Saturday'].include?(d)
